...
class CardList extends React.Component {
    ...
    Sorting = () => {
        if ($('#sort').val() === 'PriceD') {
            this.props.resultsArray.sort((a, b) => a.ratePlan.price.current > b.ratePlan.price.current ? -1 : 0
                , this.setState({}))
        }
        if ($('#sort').val() === 'PriceA') {
            this.props.resultsArray.sort((a, b) => a.ratePlan.price.current < b.ratePlan.price.current ? -1 : 0
        ...
        if ($('#sort').val() === 'RateD') {
            this.props.resultsArray.sort((a, b) => a.starRating > b.starRating ? -1 : 0
                , this.setState({}))
        }
        if ($('#sort').val() === 'RateA') {
            this.props.resultsArray.sort((a, b) => a.starRating < b.starRating ? -1 : 0
        ...
        if ($('#sort').val() === 'Rate2D') {
            this.props.resultsArray.sort((a, b) => a.guestReviews.unformattedRating > b.guestReviews.unformattedRating ? -1 : 0
                , this.setState({}))
        }
        if ($('#sort').val() === 'Rate2A') {
            this.props.resultsArray.sort((a, b) => a.guestReviews.unformattedRating < b.guestReviews.unformattedRating ? -1 : 0
        ...
        if ($('#sort').val() === 'ReviewsD') {
            this.props.resultsArray.sort((a, b) => a?.guestReviews?.total > b?.guestReviews?.total ? -1 : 0
                , this.setState({}))
        }
    ...
    render() {
        ...
        return (
            <div >
                ...
                {
                    this.props.resultsArray.length ?
                        this.props.resultsArray.map((data, i) => {
